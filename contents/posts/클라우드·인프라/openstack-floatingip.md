...
| **단계**| **레이어**| **동작**|
| -------------------- | ------------- | --------------------------------------------------------------- |
| 1. 목적지 판단 | **L3**| 목적지 IP가 같은 subnet인지 계산 (subnet mask AND 연산) |
| 2. ARP 대상 결정 | **L3**| 같은 subnet → 목적지 IP / 다른 subnet → 게이트웨이 IP |
...
> **1~2단계는 L3 로직**— "누구에게 ARP할지"를 결정하는 건 라우팅(L3)이고, ARP 자체는 L2
