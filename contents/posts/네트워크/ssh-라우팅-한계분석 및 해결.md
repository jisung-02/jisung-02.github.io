...
tags:
  - 네트워크
  - ssh
description: 하나의 소켓 주소에서 여러 서비스를 제공하기 어려운 SSH의 한계와 우회 방법
---

일반적인 SSH 연결에는 HTTP의 `Host` 헤더처럼 접속 대상 도메인을 전달하는 필드가 없다. 따라서 하나의 IP와 포트로 들어온 연결을 도메인 이름만 보고 서로 다른 VM으로 분기하기 어렵다. 이 글에서는 이 제약과 여러 VM에 접근하는 방법을 정리한다.
...
## SSH에서 도메인 기반 분기가 어려운 이유

#### 1. 라우팅 정보가 없음

SSH 전송 계층의 초기 핸드셰이크에는 HTTP의 `Host`나 TLS의 SNI에 해당하는 목적지 호스트명 필드가 없다.
...
SSH를 종료하지 않고 그대로 전달하는 프록시는 백엔드와 핸드셰이크를 이어 가야 한다. 하지만 초기 메시지에는 목적지 도메인이 없으므로, 연결을 받은 뒤 기다리더라도 도메인 기반 분기에 필요한 정보가 생기지 않는다.

#### 3. 암호화로 인해 중간에서 읽을 수 없음

TLS Handshake의 SNI처럼 평문으로 노출되는 라우팅 정보가 SSH에는 없다. 암호화가 시작되면 프록시가 내용을 읽을 수 없다.

따라서 아래 구조에서 목적지 도메인만으로 백엔드를 고르려면 별도의 라우팅 방식이 필요하다.
...
## 우회 방법
- 아래 우회 방법은 하나의 호스트에서 내부 또는 연결된 VM에 접속하는 상황을 기준으로 한다.
...
> **프록싱 vs 포트 포워딩**: 프록싱은 동적으로 목적지를 결정하지만, 포트 포워딩은 고정된 목적지로 트래픽을 보내는 파이프와 같다. 동적으로 여러 목적지를 오갈 때는 프록싱이 적합하고, 그렇지 않은 경우에는 두 방식이 동일하게 동작한다.
...
> **Port 기반 분리(방법 1)와의 관계**: 포트 기반 분리는 외부 접속 주소를 나누는 방식이고, NAT/L4 프록시는 이를 구현하는 수단 중 하나다. 외부 포트가 서로 달라도 내부 VM의 SSH 포트는 모두 22번으로 유지할 수 있다. 레이턴시는 실제 구성과 부하에 따라 달라진다.
...
프록시가 SSH 서버 역할을 맡아 클라이언트를 인증한 뒤, 허용된 백엔드로 새 SSH 연결을 만드는 방식이다.

#### 구현 방식

- Client → Proxy (SSH 종료) → Proxy → VM (새 SSH 연결)
- 암호화된 SSH 인증 정보를 일반 L4 프록시가 그대로 읽을 수는 없다. 프록시에서 SSH 연결을 종료해야 한다.
- SSH hook(`UserAuth` 단계)을 활용해 인증 정보를 읽고 분기할 수도 있다.

#### 목적지 선택: 공개키만으로는 부족할 수 있음

같은 사용자가 여러 VM에 접속한다면 공개키 외에 목적지를 구분할 정보가 필요하다. VM마다 별도 키를 만드는 것은 한 가지 설계일 뿐, SSH의 필수 조건은 아니다.
...
내가 개인 프로젝트에서 사용한 방식이다. 해당 구조에서는 SSH 요청 후 인증 절차를 거치고, 중간에 SSH 연결을 담당하는 별도의 서버를 추가해 접속할 VM을 선택할 수 있게 했다.
```
          ┌────────────┐
Client →  │ SSH Proxy  │ → VM1
          │ (Process)  │ → VM2
...
- SSH 위에 별도의 대화형 목적지 선택 절차가 추가되므로 일반 SSH 자동화 도구와 연동하려면 추가 처리가 필요하다.
...
- [RFC 4253 — SSH 전송 계층](https://www.rfc-editor.org/rfc/rfc4253.html)
- [RFC 4252 — SSH 인증](https://www.rfc-editor.org/rfc/rfc4252.html)
