...
## 핵심 정리 (시험 포인트)
- TCP/IP는 원래 **4계층**(Host-to-network·Internet·Transport·Application), OSI 대응 시 **5계층**.
- TCP/IP는 L1·L2를 표준화하지 않고 **L3(IP)부터 관리**한다.
- Network 계층 보조 프로토콜 **ARP(IP→MAC), RARP(MAC→IP, DHCP로 대체), ICMP(오류/제어, ping), IGMP(멀티캐스트 그룹 관리)**.
- IP = connectionless·unreliable / **TCP = 연결·신뢰·순서·재전송(3-way handshake)**/ UDP = 비연결·비신뢰·빠름 / SCTP = 메시지·멀티스트림·멀티홈.
- 주소 4종 ↔ 계층: **Specific(App)·Port(Transport, 16bit)·Logical IP(Network)·Physical MAC(Data Link, 48bit)**.
- **MAC은 hop-by-hop으로 매 홉 변경(로컬 의미), IP는 end-to-end로 끝까지 유지.**Port(16bit)는 프로세스 식별.
