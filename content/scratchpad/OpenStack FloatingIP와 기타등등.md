## 문제 정의
- 각 vm마다 floating IP를 할당해야하는 문제
- 현재 내부 공유기는 총 254개의 ip만 사용 가능한 상황
...
```
-> 이건 그냥 일반 TCP/HTTP 연결로 해결
### 3. 컨트롤러 -> VM
```
proxy → qrouter → VM
```
-> 핵심 가정: qrouter가 controller에 있으므로 여기서 routing 가능할 것으로 추정
...
-> 현재 qrouter-namespace에서 네트워크 요청을 수행할 경우 문제 없음
...
## 또 다른 생각
- 현재 10.0.0.x 대역 접속 실패는 192대역의 host가 같은 대역이 아니므로 게이트웨이로 보내도 qrouter가 host에게 유효한 게이트웨이로 나가지 않아서 생기는 문제
	- qrouter를 host가 인지하게 하면 됨 - 그러나 이는 Openstack 재시작마다 재설정
	- 기존 openstack이 의도한 바가 아니므로 좋은 아이디어는 X
...
# 관련 개념
## Subnet
IP 주소를 일정 범위로 나눈 네트워크 단위
```
10.0.0.0/24
→ 10.0.0.1 ~ 10.0.0.254
```
-> L3 라우팅의 판단 기준 (직접 전송할 것인지 라우터로 보낼 것인지 결정)

설명
- IP + netmask 로 범위 결정
- 같은 subnet이면 직접 통신(ARP)
- 다른 subsnet이면 라우터 필요

IP는 항상 [Network Prefix | Host IP] 로 나뉨
```
192.168.1.10 /24
→ Network: 192.168.1
→ Host: 10
...
```
- 1은 네트워크, 0은 호스트
- `ip/24` 같은 표기법은 CIDR (Classless Inter-Domain Routing) 표기법, 이는 네트워크 자리를 나타냄

#### Classful vs CIDR

원래 IP 주소는 **클래스** 단위로 네트워크 크기가 고정되어 있었음 (Classful)
...
- 문제: Class B 할당 받으면 65534개 호스트 자리 생기는데 실제로 다 못 씀 → IP 낭비
- 해결: **CIDR** — /8~/32 아무 단위로나 자유롭게 나눔 (Classless = 클래스 없음)
- 공유기가 `192.168.0.N` 대역인 이유: Class C 사설 대역 (`192.168.0.0/24`) 기본값이 굳어진 것
...
- 결과가 해당 서브넷의 **네트워크 주소** (호스트에 할당 불가)
- host 비트를 모두 1로 만든 주소는 **브로드캐스트 주소** (마찬가지로 할당 불가)
...
매번 브로드캐스트하지 않고 한번 배운 매핑을 캐시에 저장해 재사용
```
ip neigh
...
- ARP는 같은 L2 세그먼트(링크) 안에서만 동작 — 라우터 너머 원격 호스트 MAC은 못 찾음
- IPv6에서는 ARP 대신 **Neighbor Discovery (ND)** 사용
...
qrouter namespace도 Linux 네트워크 스택이므로 ARP를 그대로 사용
- VM으로 패킷 내보내기 전 VM의 MAC 주소 조회 필요
- qrouter namespace 안에도 neighbor table 생성됨

## VLAN
물리 네트워크를 논리적으로 분리하는 기술
